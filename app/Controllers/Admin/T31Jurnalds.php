...
class T31Jurnalds extends \App\Controllers\GoBaseResourceController { 
    ...
    protected $modelName = T31JurnaldModel::class;
    ...
    public function index() {
        ...
        $viewData = [
                ...
                'pageSubTitle' => lang('Basic.global.ManageAllRecords', [lang('T31Jurnalds.jurnald')]),
                't31Jurnald' => new T31Jurnald(),
                'usingServerSideDataTable' => true,
                
            ];

		$viewData['akunList'] = $this->getAkunListItems();


        $viewData = array_merge($this->viewData, $viewData); // merge any possible values from the parent controller class

        return view(static::$viewPath.'viewJurnaldList', $viewData);
    }
    ...
    public function create()
    {
        if ($this->request->isAJAX()) {
            ...
            try {
                ...
                $userFriendlyErrMsg = lang('Basic.global.persistErr1', [mb_strtolower(lang('T31Jurnalds.jurnald'))]);
                if (isset($dbError['code']) && $dbError['code'] == 1062) :
                    $userFriendlyErrMsg .= PHP_EOL.lang('Basic.global.persistDuplErr', [mb_strtolower(lang('T31Jurnalds.jurnald'))]);
                endif;
            ...
            $data = [ 'success' => $result ];
            return $this->respondCreated($data, lang('Basic.global.saveSuccess', [mb_strtolower(lang('T31Jurnalds.jurnald'))]).'.');
        } else {
    ...
    public function edit($id = null)
    {
        if (!$found = $this->model->find($id)) {
            return $this->failNotFound(lang('Basic.global.notFoundWithIdErr', [mb_strtolower(lang('T31Jurnalds.jurnald')), $id]));
        }
    ...
    public function update($id = null)
    {
        if ($this->request->isAJAX()) {
            ...
            try {
                ...
                $userFriendlyErrMsg = lang('Basic.global.persistErr3', [mb_strtolower(lang('T31Jurnalds.jurnald')), 'ID', $id]);
            ...
            $data = [ 'success' => $result ];
            return $this->respondUpdated($data, lang('Basic.global.updateSuccess', [mb_strtolower(lang('T31Jurnalds.jurnald'))]).'.', [$id]);
        } else {
            $this->failUnauthorized();
        }
    }
    ...
    public function delete($id = null)
    {
        if (!$this->model->delete($id)) {
            ...
            return $this->failNotFound(lang('Basic.global.deleteError', [mb_strtolower(lang('Jurnald.jurnald'))]));
        }

        $message = lang('Basic.global.deleteSuccess', [mb_strtolower(lang('Jurnald.jurnald'))]);
    ...
    public function datatable() {
        if ($this->request->isAJAX()) {
            ...
            $order = T31JurnaldModel::SORTABLE[$requestedOrder > 0 ? $requestedOrder : 1];
	...
	protected function getAkunListItems($selId = null) { 

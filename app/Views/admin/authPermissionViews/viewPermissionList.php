	...
	<div class="col-md-12">
		...
		<div class="card card-info">
			<div class="card-header">
				<h3 class="card-title"><?=lang('AuthPermissions.permissionList') ?></h3>
			...
			<div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>
					...
					<table id="tableOfPermission" class="table table-striped table-hover" style="width: 100%;">
						<thead>
							<tr>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
								<th><?=lang('AuthPermissions.id')?></th>
								<th><?= lang('AuthPermissions.name') ?></th>
								<th><?= lang('AuthPermissions.description') ?></th>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
							</tr>
						</thead>
						<tbody>
			...
			<div class="card-footer">
				<div class="float-right">
                    <button type="button" class="btn btn-block btn-primary" id="btnAddRecord" data-toggle="modal" data-target="#modalPermissionForm">
                        <?=lang('Basic.global.addNew').' '.lang('AuthPermissions.permission') ?>
                    </button>
				<div> <!--./ float-right-->
			</div><!--//.card-footer -->
		</div><!--//.card -->
...
<?=$this->section('footerAdditions') ?>
	<?=$this->include('admin/authPermissionViews/_permissionFormAdditions') ?>
<?=$this->endSection() ?>


<?=$this->section('additionalInlineJs') ?>
    
            const lastColNr = $('#tableOfPermission').find("tr:first th").length - 1;
            const actionBtns = function(data) {
                return `<td class="text-right py-0 align-middle">
                        <div class="btn-group btn-group-sm">
                            <button class="btn btn-sm btn-warning btn-edit mr-1" data-bs-toggle="modal" data-bs-target="#modalPermissionForm" data-id="${data.id}"><i class="fas fa-pencil-alt"></i></button>
                            <button class="btn btn-sm btn-danger btn-delete ml-1" data-id="${data.id}"><i class="fas fa-trash"></i></button>
                        </div>
                        </td>`;
            };
            theTable = $('#tableOfPermission').DataTable({
                processing: true,
                serverSide: true,
                autoWidth: true,
                responsive: true,
		...
		"buttons": [
			'copy', 'csv', 'excel', 'print', {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= route_to('dataTableOfPermission') ?>',
                    method: 'POST',
...
$(document).on('click', '#btnAddRecord', () => {
    $('#modalPermissionLabel').html("Add a New Permission");
    $('#permissionForm').find('input[name="form_action"]').val('add');
    $('#permissionForm').find('input[name="id"]').val('');
    // call select2 functions here if applicable
});

$('#modalPermissionForm').on('hidden.bs.modal', function() {
    $(this).find('#permissionForm')[0].reset();
    $('.text-danger').remove();
    $('.is-invalid').removeClass('is-invalid');
});

$(document).on('click', '#btnSavePermission', () => {
    $('.text-danger').remove();
    $('.is-invalid').removeClass('is-invalid');
    const theForm = $('#permissionForm');
    ...
    let u, fm;
    if (fa == 'add') {
        u = '<?= route_to('ajaxCreatePermission') ?>';
        fm = 'POST';
    } else {
        u = `<?= route_to('permissionList') ?>/${ $('#id').val() }/update`;
        fm = 'PUT';
    }
...
function getDataToEdit(sender) {
    $.ajax({
        url: `<?= route_to('permissionList') ?>/${sender}/edit`,
        method: 'GET',
    }).done((response) => {
        const editForm = $('#permissionForm');
        ...
        $('#modalPermissionLabel').html("Edit Permission");
...
$(document).on('click', '.btn-delete', function(e) {
        Swal.fire({
            title: '<?= lang('Basic.global.sweet.sureToDeleteTitle', [mb_strtolower(lang('AuthPermissions.permission'))]) ?>',
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= route_to('permissionList') ?>/${dataId}`,
                    method: 'DELETE',
                }).done((data, textStatus, jqXHR) => {
                    Toast.fire({
                        icon: 'success',
